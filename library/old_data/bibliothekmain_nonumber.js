...
const REPLACE = {"Ä" : "AE", "Ö" : "OE", "ß" : "SS", "Ü" : "UE", "Â" : "A", "Ê" : "E", "Î" : "I", "Ô" : "O", "Û" : "U", "Á" : "A", "É" : "E", "Í" : "I", "Ó" : "O", "Ú" : "U", "À" : "A", "È" : "E", "Ì" : "I", "Ò" : "O", "Ù" : "U"};
const REPLACE_KEYS = Object.keys(REPLACE);
...
function normalizeString(s) {
	s = s.toUpperCase();
	for (i=0; i<REPLACE_KEYS.length; i++) {
		s = s.replace(REPLACE_KEYS[i], REPLACE[REPLACE_KEYS[i]]);
	}
	return s;
}

function cmpTitle(a,b) {
	at = normalizeString(a.title); bt = normalizeString(b.title);
	if (at != bt)
		return 2*(at>bt)-1;
	aa = normalizeString(a.author); ba = normalizeString(b.author);
	if (aa != ba)
		return 2*(aa>ba)-1;
	ay = normalizeString(a.year); by = normalizeString(b.year);
	if (ay != by)
		return 2*(ay<by)-1;
	al = normalizeString(a.language); bl = normalizeString(b.language);
	if (al != bl)
		return 2*(al>bl)-1;
	return 0;
}
...
*/
function Book(author, title, year, publisher, ISBN,
		language, location) {
	this.author = author;
	this.title = title;
	this.year = year;
